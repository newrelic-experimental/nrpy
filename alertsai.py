...
def get_all_policy_conditions(nr_user_api_key, accountId, policyId, policyName, nextCursor):
    ...
    while True:
        ...
        if 'error' in result:
            logger.error(json.dumps(result))
            logger.info("policyId " + str(policyId) + " is an invalid policy.")
            ...
            if not conditions['nrqlConditions']:
                logger.info("policyId " + str(policyId) + " has no conditions.")
        ...
        # Break the loop if there is no next cursor
        if not nextCursor:
            logger.info("policyId " + str(policyId) + " - All conditions have been retrieved.")
            break
